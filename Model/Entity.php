...
class Entity{
  ...
  function getAll($parents = [], $children = []){
		$list = $this->db->query('select * from '.$this->tn)->fetchAll();
		$catalogs = [];
		foreach($parents as $parentColumn => $format){
		  $catalogs[$parentColumn] = $this->getParent($parentColumn, $format);
		}
		return array_map(function($line)use($catalogs){
		  foreach($catalogs as $column => $catalog) {
		    $key = $line[$column];
		    $line[$column] = $catalog[$key];
		  }
		  return $line;
		}, $list);
	}
	...
	//FIXME!! children
	function get($id, $children = [], $default = true, $callback = false){
		$query = 'select '.$this->columns.' from '.$this->tn.(is_array($id) ? $this->filters($id) : ' where '.$this->schema['pk'].'=?');
		$lines = $this->db->query($query, $id)->fetchAll($callback);
	...
	function getParent($parentColumn, $displayData = false){
    key_exists($parentColumn, $this->schema['parents']) or die('No such column');
    return (new Entity(key($this->schema['parents'][$parentColumn])))->catalog($displayData);
	}
	function catalog($name = false){//TODO optional filter for lists (create requires all)
		return $this->db->query('select * from '.$this->tn)->fetchAll(function($row)use($name){
			return [$row[$this->schema['pk']]=> $name ? $name($row) : current($row)];
		});
	}
	function delete($id){
		$this->db->start();
